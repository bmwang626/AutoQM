...
from arkane.encorr.corr import (assign_frequency_scale_factor,
                                get_atom_correction, get_bac)
from arkane.modelchem import CompositeLevelOfTheory, LevelOfTheory
from rmgpy import constants
from rmgpy.qm.qmdata import QMData
from rmgpy.qm.symmetry import PointGroupCalculator
from rmgpy.molecule.element import get_element
from rmgpy.statmech import (Conformer, HarmonicOscillator, IdealGasTranslation,
...
def get_lot_and_freq_scale(energy_level: str,
    ...
    energy_level = LevelOfTheory(method=energy_method, basis=energy_basis, software=energy_software)

    # Get freq level
    freq_method, freq_basis = freq_level.split('/')
    freq_level = LevelOfTheory(method=freq_method, basis=freq_basis, software=freq_software)
...
def parse_command_line_arguments(command_line_args=None):
    ...
    parser.add_argument('--csv_path', 
                        type=str, 
                        help='.csv file path containing parsed results')
    parser.add_argument('--energy_level', 
                        type=str, 
                        help='Energy level of theory')
    parser.add_argument('--freq_level',
                        type=str,
                        default=None,
                        help='Frequency level of theory')
    parser.add_argument('--freq_scale', 
                        type=float, 
                        help='Optional frequency scale factor to use')
    parser.add_argument('--energy_software',
                        type=str,
                        help='Energy software')   
    parser.add_argument('--freq_software',
                        type=str,
                        help='Frequency software')
...
def xyz_str_to_coords(xyz_str):
    ...
    coords = [[float(coord) for coord in line.split()[1:]]for line in xyz_lines if line]
