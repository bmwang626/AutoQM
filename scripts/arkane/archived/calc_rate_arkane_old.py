...
from utils import get_lot_and_freq_scale, get_rmg_conformer, parse_command_line_arguments
...
def get_rmg_conformer_from_df(
    ...
    for spc_label in ["ts", "r1", "r2", "p1", "p2"]:
        ...
        coords = xyz[:, 3:]
        atom_numbers = xyz[:, 1]
        mass = (sum([get_element(int(atom_number)).mass for atom_number in atom_numbers])/constants.Na, 'kg')
...
def calc_rate_coefficient(row,
    ...
    ts, r1, r2, p1, p2 = get_rmg_conformer_from_df(row, freq_scale, energy_level, freq_level, energy_software=energy_software, freq_software=freq_software,
                                                   use_atom_corrections=True, use_bond_corrections=False)
...
def main():
    ...
    df[['fwd_w_tunnel_A', 'fwd_w_tunnel_Ea', 'fwd_wo_tunnel_A', 'fwd_wo_tunnel_Ea',
        'rev_w_tunnel_A', 'rev_w_tunnel_Ea', 'rev_wo_tunnel_A', 'rev_wo_tunnel_Ea',]] = None

    for idx, row in df.iterrows():

        fwd_ks_w_tunnel, fwd_ks_wo_tunnel, rev_ks_w_tunnel, rev_ks_wo_tunnel = calc_rate_coefficient(
            row,
            Temps,
            freq_scale,
            energy_level,
        ...
        df.loc[idx, ['fwd_w_tunnel_A', 'fwd_w_tunnel_Ea', 'fwd_wo_tunnel_A', 'fwd_wo_tunnel_Ea',
                    'rev_w_tunnel_A', 'rev_w_tunnel_Ea', 'rev_wo_tunnel_A', 'rev_wo_tunnel_Ea',]] = [fwd_ks_w_tunnel_A, fwd_ks_w_tunnel_Ea, fwd_ks_wo_tunnel_A, fwd_ks_wo_tunnel_Ea,
                                                                                                    rev_ks_w_tunnel_A, rev_ks_w_tunnel_Ea, rev_ks_wo_tunnel_A, rev_ks_wo_tunnel_Ea,]
