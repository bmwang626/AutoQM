...
def get_rmg_conformer_from_logs(
    label,
    energy_log_path,
    freq_log_path,
    level_of_theory,
    ...
    conformer, unscaled_frequencies = freq_log.load_conformer(
        symmetry=external_symmetry,
...
def get_rmg_conformer(
    ...
    external_symmetry, optical_isomers = get_symmetry(
        coords,
        numbers,
    )
...
def get_rmg_conformer_from_yml(path, freq_scale=None):
    ...
    level_of_theory, freq_scale = get_lot_and_freq_scale(
        energy_level=energy_level, freq_level=freq_level, freq_scale=freq_scale
    )
    ...
    for hash_id in results["valid_conformer_hash_ids"]:
        ...
        mass = (
            sum([get_element(symbol).mass for symbol in xyz["symbols"]]) / constants.Na,
            "kg",
        )

        e_electronic = (
            cur_conformer["energy"]["sp_include_solv_correction"] * 2625500
        )  # hartree to J/mol
...
def calc_ms_tst_rate_coefficient(
    reactants_path,
    products_path,
    ts_path,
    Temps,
    ...
    reactants = [
        get_rmg_conformer_from_yml(path, freq_scale) for path in reactants_path
    ]
    ...
    for idx in range(Temps.shape[0]):
        T = Temps[idx]
        ks_wo_tunnel[idx] = (
            constants.kB * T / constants.h * Q_TS[idx] / Q_reac[idx]
        ) * np.exp(-dE0 / constants.R / T)
...
def parse_command_line_arguments(command_line_args=None):
    ...
    parser.add_argument(
        "--ts", type=str, default=None, help="Final project info file name for TS"
    )
    parser.add_argument(
        "--freq_scale",
        type=float,
    ...
    parser.add_argument(
        "--index", type=int, default=0, help="Optional reaciton index to use"
    )
...
def main():
    ...
    Temps = np.arange(275.0, 351.0, 1.0)
    ...
    fwd_ks_w_tunnel, fwd_ks_wo_tunnel = calc_ms_tst_rate_coefficient(
        reactants_path=fwd_reactants_path,
    ...
    rev_ks_w_tunnel, rev_ks_wo_tunnel = calc_ms_tst_rate_coefficient(
        reactants_path=fwd_products_path,
...
"""
    ...
    fwd_ks_w_tunnel_fit = Arrhenius().fit_to_data(
        Temps, fwd_ks_w_tunnel, kunits=k_units_dict[fwd_rxn_order], three_params=False
    )
    with open("fwd_ks_w_tunnel_fit.txt", "w") as f:
        f.write(
            rmg_lib_str.format(
                index=args.index,
                r=fwd_reactant_names,
                p=fwd_product_names,
                kinetics=fwd_ks_w_tunnel_fit,
                freq_scale=freq_scale,
            )
        )

    fwd_ks_wo_tunnel_fit = Arrhenius().fit_to_data(
        Temps, fwd_ks_wo_tunnel, kunits=k_units_dict[fwd_rxn_order], three_params=False
    )
    ...
    rev_ks_w_tunnel_fit = Arrhenius().fit_to_data(
        Temps, rev_ks_w_tunnel, kunits=k_units_dict[rev_rxn_order], three_params=False
    )
    with open("rev_ks_w_tunnel_fit.txt", "w") as f:
        f.write(
            rmg_lib_str.format(
                index=args.index,
                r=fwd_product_names,
                p=fwd_reactant_names,
                kinetics=rev_ks_w_tunnel_fit,
                freq_scale=freq_scale,
            )
        )

    rev_ks_wo_tunnel_fit = Arrhenius().fit_to_data(
        Temps, rev_ks_wo_tunnel, kunits=k_units_dict[rev_rxn_order], three_params=False
    )
